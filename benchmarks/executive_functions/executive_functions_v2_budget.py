"""Executive Functions Benchmark v2 — Budget Edition (~$20 of $50 quota)
...
def e2_inhibitory_control(llm):
    ...
    for gi, gs in enumerate(GOLD_SET):
        dramatic_text = gs["transforms"]["dramatic"]
        try:
    ...
    for probe in PROBES_INHIBITION:
        ...
        dramatic_text = probe.get("dramatic_text", probe["anchor"])
        try:
...
def e4_working_memory(llm):
    ...
    print("  Tiers: 2-party (6), 4-party (5), 6-party (3 composite), 8-party (2 composite)")
    print("-" * 60)
    ...
    # Tier-4: Dear Abby with more characters
    tier4_candidates = [s for s in DEAR_ABBY if len(s["text"]) > 300]
    tier4_scenarios = []
    for s in tier4_candidates[:5]:
    ...
    for tier in [2, 4, 6, 8]:
        if tier_counts[tier]:
            ...
            # Score: consistency (how reliably it identifies same count)
            tier_scores[tier] = {
                "avg_count": avg_count,
                "consistency": avg_con,
            ...
            tier_scores[tier] = {"score": 0.0, "n_scenarios": 0, "avg_count": 0, "consistency": 0}
    ...
    print(f"  {'Tier':<10} {'Score':>8} {'AvgCount':>10} {'Consist':>10} {'N':>4}")
    print(f"  {'-'*42}")
    for tier in [2, 4, 6, 8]:
        ts = tier_scores[tier]
        if ts["n_scenarios"] > 0:
            print(f"  Tier-{tier:<4} {ts['score']:>7.3f} {ts['avg_count']:>9.1f} "
                  f"{ts['consistency']:>9.3f} {ts['n_scenarios']:>4}")
