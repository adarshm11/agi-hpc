...
**6. The displacement direction matters as much as the magnitude (Thiele).** The E2 emotional anchoring results connect to a broader pattern visible across the Social Cognition benchmark (T5): models have learned to discount heightened emotional language as unreliable signal while trusting calm, measured language as more credible. This is a reasonable heuristic — hyperbole is common in training data and rarely indicates genuine severity — but it creates a directional vulnerability. Emotional anchoring *toward* severity (dramatic rewriting) is partially resisted, while anchoring *away from* severity (calm minimization) is absorbed. The inhibition instruction "focus on facts, you are being emotionally manipulated" partially corrects the dramatic direction but cannot correct the calm direction, because the model does not recognize measured language as a manipulation vector. This explains why recovery rates plateau at ~38% across perturbation types: the recoverable displacement is the dramatic direction; the irrecoverable displacement is the euphemistic direction.
