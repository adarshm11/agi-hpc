"""Executive Functions Benchmark v1b — Budget Edition ($50 quota)
...
Expected runtime: ~1-2 hours (adaptive, 4 models × ~230 calls each).
"""
...
print("=" * 70)
print("EXECUTIVE FUNCTIONS BENCHMARK v1b BUDGET (adaptive parallelism)")
...
def e1_cognitive_flexibility(llm):
    ...
    Budget: 12 scenarios x 5 calls = ~60 calls
    """
    print("\n[E1] COGNITIVE FLEXIBILITY")
    print("  Testing genuine framework-switching vs. relabeling")
    print("  Budget: 12 scenarios x 5 calls = ~60 calls")
    print("-" * 60)
...
def e2_inhibitory_control(llm):
    ...
    Budget: 15 scenarios x 3 conditions = 45 calls
    """
    print("\n[E2] INHIBITORY CONTROL")
    print("  Testing resistance to emotional anchoring")
    print("  Budget: 15 scenarios x 3 conditions = 45 calls")
    print("-" * 60)

    scenarios = DEAR_ABBY[:15]  # budget: reduced from 30
...
def e3_planning(llm):
    ...
    Budget: 12 scenarios x 4 calls = 48 calls
    """
    print("\n[E3] PLANNING / MULTI-STEP REASONING")
    print("  Causal chain tracing, inflection points, and what-if analysis")
    print("  Budget: 12 scenarios x 4 calls = 48 calls")
    print("-" * 60)
...
def e4_working_memory(llm):
    ...
    Budget: 18 scenarios x 3 calls = 54 calls
    Tiers: 2-party (8), 4-party (5), 6-party composite (3), 8-party composite (2)
    """
    print("\n[E4] WORKING MEMORY")
    print("  Tracking morally relevant parties at scale")
    print("  Budget: 18 scenarios x 3 calls = 54 calls")
    print("-" * 60)

    # --- Build tiered scenarios ---

    # Tier-2: Simple dyadic scenarios (8 letters) — budget
    tier2_scenarios = []
    for s in DEAR_ABBY[:8]:  # budget: reduced from 15
    ...
    # Tier-4: Select Dear Abby letters with 4+ named parties (5 letters) — budget
    ...
    tier4_scenarios = []
    for s in tier4_candidates[:5]:  # budget: reduced from 10
    ...
    # Tier-6: Composite scenarios (combine 2 letters with bridge text) — budget
    tier6_scenarios = []
    tier6_pairs = [
    ...
    # Tier-8: Composite scenarios (combine 3 letters with bridge text) — budget
    tier8_scenarios = []
    tier8_triples = [
        (DEAR_ABBY[30], DEAR_ABBY[31], DEAR_ABBY[32]),
        (DEAR_ABBY[33], DEAR_ABBY[34], DEAR_ABBY[35]),
    ]  # budget: reduced from 5 to 2
...
# Budget: 4 models × ~230 calls × $0.014 = ~$13 (within $50 quota)
