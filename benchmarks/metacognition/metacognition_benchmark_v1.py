"""Metacognition Benchmark v1 — Hybrid AITA + Dear Abby, Six Sigma Edition
...
Expected runtime: ~2-4 hours (adaptive, 5 models x ~540 calls each).
"""
...
print("=" * 70)
print("METACOGNITION BENCHMARK v1 (adaptive parallelism)")
...
random.seed(42)
PER_CLASS = 25  # 25 per class = 100 AITA scenarios for M1/M2
...
def m1_calibration(llm):
    ...
    print("  100 AITA scenarios x 1 call + 50 repeats for stability")
    print("-" * 60)

    scenarios = AITA_SCENARIOS[:100]
    ...
    # --- Phase 2: 50 repeats on random subset for stability ---
    repeat_scenarios = random.sample(list(range(min(50, len(scenarios)))), min(50, len(scenarios)))
...
def m2_knowing_unknowns(llm):
    ...
    print("  25 clear-cut + 25 ambiguous x 3 repeats = 150 calls")
    print("-" * 60)
...
def m3_self_monitoring(llm):
    ...
    print("  20 scenarios x 7 dimension calls = 140 calls")
    print("-" * 60)
...
def m4_strategy_selection(llm):
    ...
    print("  10 simple + 10 complex Dear Abby scenarios x 5 repeats = 100 calls")
    print("-" * 60)
    ...
    simple_scenarios = [s for _, s in scored[:10]]
    complex_scenarios = [s for _, s in scored[-10:]]
