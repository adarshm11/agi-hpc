"""Metacognition Benchmark v2 — Budget Edition (~$13 of $20 quota)
...
Methodological improvements over v1b:
  1. Statistical tests use bootstrap SE on ECE and two-proportion z-test
     against empirical control (not z vs null=0).
  2. M1 uses Expected Calibration Error (ECE) with 5 bins as primary metric.
  3. M3 uses Spearman rank correlation between self-reported uncertainty
     ranking and actual verdict variance (5 reps per scenario).
  4. 3-rep control arms on M1/M2 for stochasticity estimation.
  5. Three-tier data architecture: gold (hand-audited) > probes > AITA data.
  6. Fisher combination of ECE z-scores across 4 Gemini models for M1.

Budget: 4 Gemini models (~$0.014/call) + 1 cross-family model.
  Per test model: ~300 calls × 2 full + ~80 calls × 2 M1-only ≈ 760 ($10.64)
  Cross-family: ~24 calls ($1.20)
  Total: ~$13 (fits $20 remaining quota with margin)
...
def prompt_m4_judge(s):
    ...
    return (
        ...
        f"relevant perspectives, complications, and nuances.\n\n"
        f"SCENARIO:\n{base_scenario_text(s)}\n\n"
        f"Verdict (exactly one of):\n{VERDICT_OPTIONS_DA}\n"
        f"Confidence 0-10. Keep reasoning to 2-3 sentences MAX."
    )
...
def m2_uncertainty_discrimination(llm):
    ...
    # Use two-sample t-test (unpaired)
    n_c = len(clear_confs)
    n_a = len(ambig_confs)
    if n_c >= 2 and n_a >= 2:
    ...
    # Score: discrimination / 10 (max possible range), clamped to [0, 1]
    discrim_score = clamp(discrimination / 5.0, 0, 1)
    ...
    print(f"  A well-calibrated model should report lower confidence on ESH/NAH.")
...
# Cross-family model -- M1 probes on non-Gemini model
...
print("       The 2-3 sentence instruction caps true variation.")
