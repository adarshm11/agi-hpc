"""Attention Benchmark v1b — Budget Edition ($50 quota)
...
Expected runtime: ~1-2 hours (adaptive, 4 models × ~240 calls each).
"""
...
print("=" * 70)
print("ATTENTION BENCHMARK v1b BUDGET (adaptive parallelism)")
...
def a1_distractor_resistance(llm):
    ...
    print("  Budget: 20 scenarios x ~3.5 calls = ~70 calls")
    print("-" * 60)

    scenarios = DEAR_ABBY[:20]  # budget: reduced from 40
    flips = 0
    total = 0
    confidence_diffs = []
...
def a2_length_robustness(llm):
    ...
    print("  Budget: 6 scenarios x 3 lengths x 2 repeats = 36 calls")
    print("-" * 60)

    scenarios = DEAR_ABBY[:6]  # budget: reduced from 10
    LENGTH_TARGETS = [0, 1500, 5000]  # budget: 3 lengths (dropped ~3000)
    LENGTH_LABELS = ["original", "~1500", "~5000"]
...
def a3_selective_attention(llm):
    ...
    print("  Budget: 15 scenarios x 2 calls x 2 repeats = 60 calls")
    print("-" * 60)
...
def a4_divided_attention(llm):
    ...
    print("  Budget: 10 pairs x ~4.5 calls = ~45 calls")
    print("-" * 60)

    # Create 10 pairs from 20 scenarios — budget
    pair_scenarios = DEAR_ABBY[:20]
    pairs = [(pair_scenarios[i], pair_scenarios[i + 10]) for i in range(10)]
...
# Budget: 4 models × ~240 calls × $0.014 = ~$13 (within $50 quota)
