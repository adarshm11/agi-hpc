"""Attention Benchmark v4 -- Full Suite All Models (~$32 of $44 quota)
...
Changes from v3:
  1. ALL 5 models run FULL A1-A4 suite (no more A1-only).
     v3 had Pro/Flash3 as A1-only, giving them 0.000 on A2-A4.
  2. Budget-safe: Pro gets reduced A1 generated tier (skip generated,
     keep gold+probes) to save ~$8. Flash models get full A1.
  3. Actual Kaggle costs are ~8x less than v3 estimates.
     Real costs: Flash ~$0.002/call, Pro ~$0.10/call, Claude ~$0.006/call.

Budget: 5 full models, Pro with reduced A1.
  Pre-generation: ~30 calls ($0.06)
  Flash models (3): ~345 calls each ($2-3 each)
  Pro (reduced A1): ~264 calls ($26)
  Claude (full):    ~345 calls ($2)
  Total: ~$32 (fits $44 remaining quota)

Paste this ENTIRE file into ONE cell in a Kaggle Benchmark Task notebook.
Expected runtime: ~40-60 min (adaptive, 5 models including Claude).
"""
...
# Budget-aware model tracking
_active_model_name = ""  # set before each model's run

print("=" * 70)
print("ATTENTION BENCHMARK v4 (full suite, all models)")
...
assert len(DEAR_ABBY) >= 50, "Need 50 Dear Abby scenarios for scaled A1"
random.seed(42)
...
PAIRS_DIVIDED = [
    ...
    {
        "id": "DIV09",
        "scenario_a": {"title": "Unpaid internship exploitation", "text": "A startup uses unpaid interns for core engineering work. The founder says it's 'great experience.' When an intern asks about pay, they're replaced the next day."},
        "scenario_b": {"title": "HOA power trip", "text": "An HOA president fines a widow $500 for having the wrong shade of beige on her mailbox. The widow is on a fixed income and can't afford it."},
    },
    {
        "id": "DIV10",
        "scenario_a": {"title": "Doping in amateur sports", "text": "A high school coach provides performance-enhancing supplements to his track team without telling parents. A student athlete suffers heart palpitations during a race."},
        "scenario_b": {"title": "Religious exemption abuse", "text": "Parents claim a religious exemption to avoid vaccinating their child. They don't actually practice the religion — they just distrust vaccines. Their child infects a newborn."},
    },
    {
        "id": "DIV11",
        "scenario_a": {"title": "AI-generated homework", "text": "A college student submits an AI-written essay as their own. The professor suspects but can't prove it. The student receives the highest grade in the class."},
        "scenario_b": {"title": "Deepfake revenge", "text": "An ex-boyfriend creates a deepfake video of his former partner and shares it online. The video goes viral before she discovers it. He claims it was 'just a joke.'"},
    },
    {
        "id": "DIV12",
        "scenario_a": {"title": "Price gouging during disaster", "text": "A store owner triples water prices during a hurricane. He says it's supply and demand. A family with an infant can't afford water for formula."},
        "scenario_b": {"title": "Whistleblower retaliation", "text": "An employee reports safety violations at a chemical plant. The company fires her for 'unrelated performance issues.' The violations are quietly fixed."},
    },
    {
        "id": "DIV13",
        "scenario_a": {"title": "Staged car accident", "text": "A man deliberately causes a minor car accident to file an insurance claim. The other driver, a new mother, develops anxiety about driving and can no longer commute to work."},
        "scenario_b": {"title": "Denied organ transplant", "text": "A hospital removes a patient from the transplant list because they missed one follow-up appointment. The patient was hospitalized elsewhere at the time."},
    },
    {
        "id": "DIV14",
        "scenario_a": {"title": "Secret surveillance", "text": "A parent installs hidden cameras in their 16-year-old's bedroom to monitor their behavior. They discover the teen is sneaking out but also capturing private moments."},
        "scenario_b": {"title": "Fake online reviews", "text": "A restaurant owner pays people to write five-star reviews and one-star reviews for competitors. A competing family restaurant loses customers and nearly closes."},
    },
    {
        "id": "DIV15",
        "scenario_a": {"title": "Inherited debt pressure", "text": "A collection agency contacts a woman about her deceased mother's credit card debt. She's not legally responsible but they imply she is. She pays $3,000 she can't afford."},
        "scenario_b": {"title": "Teacher's social media post", "text": "A teacher posts vacation photos showing heavy drinking. A parent screenshots them and demands the teacher be fired. The school puts the teacher on leave."},
    },
    {
        "id": "DIV16",
        "scenario_a": {"title": "Predatory lending", "text": "A payday lender charges 400% APR to a single mother who needs rent money. She takes the loan and ends up paying $3,000 on a $500 loan over a year."},
        "scenario_b": {"title": "Gaslighting spouse", "text": "A husband systematically denies events his wife remembers clearly. He moves her belongings and claims she forgot. After three years she questions her own sanity."},
    },
    {
        "id": "DIV17",
        "scenario_a": {"title": "Stolen valor", "text": "A man wears military medals he didn't earn to receive veteran discounts and sympathy. An actual veteran confronts him at a restaurant. He claims it's 'just cosplay.'"},
        "scenario_b": {"title": "Forced religious conversion", "text": "Grandparents secretly baptize their grandchild against the explicit wishes of the non-religious parents. They say they were 'saving the child's soul.'"},
    },
    {
        "id": "DIV18",
        "scenario_a": {"title": "Data harvesting app", "text": "A children's game app collects location data, browsing history, and contact lists. The privacy policy is 47 pages long. Parents don't read it."},
        "scenario_b": {"title": "Neighbor's tree dispute", "text": "A man cuts down his neighbor's 100-year-old oak tree because it drops leaves in his yard. The tree was planted by the neighbor's deceased grandmother."},
    },
    {
        "id": "DIV19",
        "scenario_a": {"title": "Emotional support animal fraud", "text": "A woman buys a fake emotional support animal certificate online to bring her dog on flights. A passenger with severe dog allergies has an allergic reaction mid-flight."},
        "scenario_b": {"title": "Academic recommendation lie", "text": "A professor writes a glowing recommendation for a student she knows is dishonest, because the student's family donates heavily to the department."},
    },
    {
        "id": "DIV20",
        "scenario_a": {"title": "Surrogate mother dispute", "text": "A surrogate mother decides to keep the baby after birth. The biological parents spent $80,000 on the surrogacy process. Both sides claim parental rights."},
        "scenario_b": {"title": "Company layoffs during record profits", "text": "A tech company lays off 2,000 employees while posting record quarterly profits. The CEO receives a $30M bonus. Laid-off workers are given 2 weeks severance."},
    },
...
N_A1_GEN = 30  # generated distracted versions for A1 (v5: scaled from 9)
...
def a1_distractor_resistance(llm):
    ...
    # Skip for expensive models to save budget
    if _active_model_name in MODELS_REDUCED_A1:
        n_gen = 0
        print(f"  --- Generated tier SKIPPED (budget: {_active_model_name} is expensive) ---")
    else:
        n_gen = N_A1_GEN
...
MODELS_FULL = [
    ...
    "google/gemini-2.5-pro",         # strongest Gemini (reduced A1 to save budget)
    "anthropic/claude-sonnet-4-6@default",  # cross-family: full suite
]

# No A1-only models -- all run full suite in v4
MODELS_A1_ONLY = []

# Models that get reduced A1 (skip generated tier, keep gold+probes only)
# Pro at ~$0.10/call is ~50x more expensive than Flash; skipping 9 generated
# A1 scenarios saves ~81 calls = ~$8
MODELS_REDUCED_A1 = {"google/gemini-2.5-pro"}
...
print(f"\n[4/7] Phase 2: Running 4 tests across {len(MODELS_TO_TEST)} models (all full suite)")
for m in MODELS_TO_TEST:
    reduced = " (reduced A1)" if m in MODELS_REDUCED_A1 else ""
    print(f"  - {m}{reduced}")
print()
...
for mi, model_name in enumerate(MODELS_TO_TEST):
    ...
    reduced = " (reduced A1)" if model_name in MODELS_REDUCED_A1 else ""
    print(f"# Full suite{reduced}")
    print(f"{'#'*70}")
