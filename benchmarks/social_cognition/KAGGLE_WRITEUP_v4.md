...
**5. The asymmetry reveals a learned heuristic about moral salience (Thiele).** Claude's directional vulnerability — absorbing euphemistic minimization (drift=−9.1) while discounting dramatic exaggeration (drift=−1.5) — is not a random failure. These models have implicitly learned that heightened emotional language is unreliable signal, a reasonable inference from training data where hyperbole is common and rarely indicates genuine severity. Conversely, they treat calm, measured language as more trustworthy. But calm and accurate are not the same thing. Harmful actions are frequently described in measured, reasonable-sounding terms by actors who benefit from the outcome reading as less severe — corporate liability framing, legal euphemism, institutional harm minimization. A model that discounts drama while trusting calm tone is vulnerable in precisely the direction where accurate moral judgment matters most: it knows what to ignore, but not fully what to distrust. This directional vulnerability is exploitable by any actor who frames harmful content in measured, professional language — the exact register used by those most practiced at minimizing harm descriptions.
