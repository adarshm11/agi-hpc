...
#SBATCH --output=build_mamba_slurm-%j.log
#SBATCH --error=build_mamba_slurm-%j.err
...
pip install -v --no-binary causal-conv1d causal-conv1d 2>&1 | tail -30

echo "=== Building mamba-ssm ==="
pip install -v --no-binary mamba-ssm mamba-ssm 2>&1 | tail -30
